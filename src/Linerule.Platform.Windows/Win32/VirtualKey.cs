...
internal static class VirtualKey
{
    ...
    public static uint FromKeyCode(KeyCode key) => key switch
    {
    ...
    public static HOT_KEY_MODIFIERS FromModifiers(Modifiers mods)
    {
        ...
        if (mods.Ctrl) { r |= HOT_KEY_MODIFIERS.MOD_CONTROL; }
        if (mods.Alt) { r |= HOT_KEY_MODIFIERS.MOD_ALT; }
        if (mods.Shift) { r |= HOT_KEY_MODIFIERS.MOD_SHIFT; }
        if (mods.Meta) { r |= HOT_KEY_MODIFIERS.MOD_WIN; }
        return r;
    }
}
